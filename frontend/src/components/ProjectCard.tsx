...
export function ProjectCard({ project, taskStats, showAI = true }: ProjectCardProps) {
  ...
  return (
    <div className="project-card">
      <Link to={`/projects/${project.id}`} className="project-card__link-wrapper">
        ...
        <div className="project-card__body">
          ...
          <div className="project-card__meta-pills">
            <span className="project-card__client-pill" title={`Client: ${clientName}`}>
            ...
            {pmName && (
              <span className="badge badge--role-pm" title={`Project Manager: ${pmName}`}>
                PM: {pmName}
              </span>
            )}
            {memberCount > 0 && (
      ...
      {showAI && (
        <div className="project-card__ai-wrapper">
          <AIProjectSummary projectId={project.id} projectName={project.name} />
        </div>
      )}
    </div>
  );
